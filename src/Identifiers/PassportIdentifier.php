...
final class PassportIdentifier implements ValidatorInterface, ParserInterface
{
    ...
    public function validate(string $input): ValidationResult
    {
        ...
        if (! ctype_alpha(substr($normalized, 0, 2))) {
            return ValidationResult::failWithReason(
        ...
        if (! ctype_digit(substr($normalized, 2))) {
            return ValidationResult::failWithReason(
                ValidationFailureReason::InvalidCharacters,
    ...
    #[\Override]
    public function parse(string $input): Passport
    {
    ...
    #[\Override]
    public function tryParse(string $input): ?Passport
    {
        try {
