...
3. Decode the birth date using the century encoding table below. Fail with `InvalidMonth` if the stored month does not fall in any known range. Fail with `InvalidDate` if the decoded date is not a real calendar date. Fail with `FutureDate` if the birth date is in the future.
...
5. Reject `00000000000`; fail with `AllZeros`.
...
<Aside type="note">
The PESEL `92060512186` is used as an example throughout the docs. Its encoded date is `1992-06-05`, gender is Female (digit 10 is `8`, an even digit), ordinal is `121` (digits 7–9).
</Aside>
