...
KRS has no publicly documented checksum algorithm. Numerik validates **format and structure only** — it cannot verify whether a given KRS number is actually assigned in the registry. For authoritative verification, query the official [KRS registry API](https://api-krs.ms.gov.pl).
</Aside>
