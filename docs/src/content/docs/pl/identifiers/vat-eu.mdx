...
VAT-EU (*Numer VAT UE*) to numer identyfikacji podatkowej stosowany w transakcjach wewnątrzunijnych. W przypadku polskich podatników składa się z dwuliterowego prefiksu kraju `PL` oraz standardowego 10-cyfrowego numeru NIP. Walidacja polega na usunięciu prefiksu i zastosowaniu algorytmu sumy kontrolnej MOD-11 używanego przez NIP.
...
// Separatory i małe litery w prefiksie są akceptowane
...
| `InvalidFormat` | `invalid_format` | Brak prefiksu `PL` lub kod urzędu skarbowego NIP to `000`. |
